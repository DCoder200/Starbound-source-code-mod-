...
void WorldGenerator::prepareTiles(WorldStorage* worldStorage, ServerTileSectorArray::Sector const& sector) {
  ...
  // get blocks that aren't shiftable from json
  auto assets = Root::singleton().assets();
  auto unshiftableJson = assets->json("/tiles/materials/unshiftable.config:usesVariant");
  //Logger::info("NEWARRAY: {}", Json(unshiftableJson).printJson());

  // convert to normal array
  List<String> unshiftable;
  auto array = unshiftableJson.toArray();
  for (auto const& value : array) {
    unshiftable.append(value.toString());
  }

  // Generate sector.
  auto tileArray = worldStorage->tileArray();

  // setup planet hue variable and a boolean flag
  MaterialHue planetHue;
  bool hueCaptured = false;


  // all tiles inside a sector
  ...
  for (int x = sectorRegion.xMin(); x < sectorRegion.xMax(); ++x) {
    for (int y = sectorRegion.yMin(); y < sectorRegion.yMax(); ++y) {
      ...
      // capture planet hue once sampling the main block
      if (!hueCaptured && blockInfo.blockBiomeIndex != NullBiomeIndex) {
        auto biome = planet->biome(blockInfo.blockBiomeIndex);
        if (biome) {
          MaterialId biomeBlock = biome->mainBlock;
          Logger::info("MAIN BLOCK: {}", biomeBlock);
          planetHue = planet->biomeMaterialHueShift(blockInfo.blockBiomeIndex, biomeBlock);
          Logger::info("PLANET HUE: {}", planetHue);
          hueCaptured = true;
        }
      }
      ...
      auto hue = planet->biomeMaterialHueShift(tile->blockBiomeIndex, 0);

      // check if material is inside unshiftables
      for (auto const& value : unshiftable) {
        if (tile->foreground == materialDatabase->materialId(value)){
            Logger::info("Unshiftable detected. Hue: {}", hue);
            tile->foregroundColorVariant = 2;
        }

        if (tile->background == materialDatabase->materialId(value)){
            tile->backgroundColorVariant = 2;
        }
      }

/*
      if (tile->foreground == materialDatabase->materialId("cobblestone")){
        //Logger::info("Cobblestone detected. Hue: {}", hue);



        if (planetHue){
            float normalizedHue = (planetHue / 240.0f) * 360.0f;
    
            // doesn't seem accurate most of the time
            if (normalizedHue < 20.0f)
                Logger::info("RED");
            else if (normalizedHue < 45.0f)
                Logger::info("ORANGE");
            else if (normalizedHue < 70.0f)
                Logger::info("YELLOW");
            else if (normalizedHue < 150.0f)
                Logger::info("GREEN");
            else if (normalizedHue < 210.0f)
                Logger::info("CYAN");
            else if (normalizedHue < 260.0f)
                Logger::info("BLUE");
            else if (normalizedHue < 320.0f)
                Logger::info("PURPLE");
            else
                Logger::info("MAGENTA");
        }
      ...
      if (tile->background == materialDatabase->materialId("cobblestone")){
        //Logger::info("Cobble found");
        tile->backgroundColorVariant = 2;
      }
*/
    }
