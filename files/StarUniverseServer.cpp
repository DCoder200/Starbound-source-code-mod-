...
Maybe<CelestialCoordinate> UniverseServer::nextStarterWorld() {
  ...
  // this is "" from json initially
  String defaultWorldCoordinate = assets->json("/universe_server.config:defaultWorldCoordinate").toString();
 
  // if it's not "" then return the coordinate
  if (!defaultWorldCoordinate.empty())
    return CelestialCoordinate(defaultWorldCoordinate);

  // if next random world exists and is done then set coordinate of next world 
  // m_ indicates an async task happening in the background
  ...
  // if a next random world doesn't exist then 
  if (!m_nextRandomizedStarterWorld) {

    // run the above async task until one is returned by it 
    m_nextRandomizedStarterWorld = m_workerPool.addProducer<CelestialCoordinate>([assets, celestialDatabase = m_celestialDatabase]() {
      ...
      // method that filters the world to decide if it's acceptable, will return a boolean, auto just means automatially assigned type for variables, takes json parameters as an argument
      auto filterWorld = [celestialDatabase](CelestialCoordinate const& coordinate, Json const& filter) {

        // filter out non landable
        ...
        // check planet biome
        if (auto biomeArray = filter.optArray("terrestrialBiome")) {
            //Logger::info("NEWARRAY: {}", Json(*biomeArray).printJson());
          ...
          if (!terrestrialParameters)
            ...
            int randomIndex = rand() % biomeArray->size();
        
            auto biome = (*biomeArray)[randomIndex];
            //Logger::info("BIOME: {}", biome.toString());
           
            // if biome is the primary biome of the planet then it's a match
            if (biome.type() == Json::Type::String && biome.toString() == terrestrialParameters->primaryBiome){
                return true;
            }
        ...
        // check what dungeon type the planet is 
        if (auto dungeon = filter.optString("floatingDungeon")) {
          ...
          // if there's no primary dungeon return false
      ...
      auto randomWorld = celestialDatabase->findRandomWorld(findParameters.getUInt("tries"), findParameters.getUInt("range"), [&](CelestialCoordinate const& coordinate) {
        ...
        // run first function above, return false if it failed to pass
        if (!filterWorld(coordinate, findParameters.get("starterWorld")))
          return false;
        
        // list all children of the coordinate
        List<CelestialCoordinate> allChildren;

        // get all the children of the coordinate (star system)
        ...
        // look for required system worlds from array
        for (auto const& requiredSystemWorld : findParameters.getArray("requiredSystemWorlds", {})) {
          bool worldFound = false; // flag to indicate the required world was found 
          for (auto const& world : allChildren) {

            // put it through the filter world check
